...
class email_rules
{
    ...
    public $send_to;
    public $path;
    public $attach_template_files = array();
    public $attach_img_files = array();
    public $rules;
    ...
    function __construct($entity_id, $item_id)
    {
        ...
        $this->send_to = array();
        $this->attach_template_files = array();
    ...
    function prepare_subitems_list($entities_id, $text)
    {
        ...
        {
            $html .= '<li>' . $text_pattern->output_singe_text($text, $entities_id, $items, array('is_email' => true, 'is_export' => true)) . '</li>';
        }
        $html .= '</ul>';
        return $html;
    }
    ...
    function prepare_send_to($rules)
    {
        ...
        switch ($rules['action_type'])
        {
            ...
            case 'schedule_send_to_email':
                ...
                {
                    $send_to = preg_split('/[\s,]+/', trim($rules['send_to_email']));
                }
                break;
            case 'insert_send_to_assigned_email':
            case 'edit_send_to_assigned_email':
    ...
    function send_insert_msg()
    {
        ...
        while ($rules = db_fetch_array($rules_query))
        {
            ...
            $text = $text_pattern->output_singe_text($rules['description'], $this->entity_id, $this->item_info, array('is_email' => true, 'hide_attachments_url' => $rules['attach_attachments'], 'path' => $this->path));
    ...
    function send_edit_msg($previous_item_info)
    {
        ...
        while ($rules = db_fetch_array($rules_query))
        {
            if ((isset($this->item_info['field_' . $rules['monitor_fields_id']]) ? $this->item_info['field_' . $rules['monitor_fields_id']] : '') == (isset($previous_item_info['field_' . $rules['monitor_fields_id']]) ? $previous_item_info['field_' . $rules['monitor_fields_id']] : ''))
            {
                continue;
            }
            if (strlen($rules['monitor_choices'] ?? ''))
    ...
    // UPDATED send() – Handles Individual Mode Fallback + Reply-To
    function send($send_to, $subject, $text, $attachments = array(), $type = '')
    {
        ...
        if ($entity_cfg->get('disable_internal_notification') != 1 && strlen($type))
        {
        ...
        $cc_raw  = $this->rules['cc_emails'] ?? '';
        $bcc_raw = $this->rules['bcc_emails'] ?? '';

        $cc_emails  = $text_pattern->output_singe_text($cc_raw, $this->entity_id, $this->item_info);
        $bcc_emails = $text_pattern->output_singe_text($bcc_raw, $this->entity_id, $this->item_info);

        $cc_list  = strlen($cc_emails) ? preg_split('/[\s,]+/', $cc_emails, -1, PREG_SPLIT_NO_EMPTY) : [];
        $bcc_list = strlen($bcc_emails) ? preg_split('/[\s,]+/', $bcc_emails, -1, PREG_SPLIT_NO_EMPTY) : [];

        // PREPARE SENDER AND REPLY-TO (Parsing variables like [6045] first)
        $sender_email_parsed = '';
        $sender_name_parsed = '';
        if(!empty($this->rules['send_from_email']))
        {
            $sender_email_parsed = $text_pattern->output_singe_text($this->rules['send_from_email'], $this->entity_id, $this->item_info);
            $sender_name_parsed  = $text_pattern->output_singe_text($this->rules['send_from_name'] ?? '', $this->entity_id, $this->item_info);
        }

        $reply_to_email_parsed = '';
        $reply_to_name_parsed = '';
        if(!empty($this->rules['reply_to']))
        {
            $reply_to_email_parsed = $text_pattern->output_singe_text($this->rules['reply_to'], $this->entity_id, $this->item_info);
            $reply_to_name_parsed  = $text_pattern->output_singe_text($this->rules['reply_to_name'] ?? '', $this->entity_id, $this->item_info);
        }


        if (!empty($this->rules['send_as_group']) && $this->rules['send_as_group'] == 1)
        {
            // GROUPED MODE
            ...
            {
                $mail->isSMTP();
                $mail->Host = CFG_EMAIL_SMTP_SERVER;
                $mail->Port = CFG_EMAIL_SMTP_PORT;
                $mail->SMTPSecure = CFG_EMAIL_SMTP_ENCRYPTION;
                if (CFG_EMAIL_SMTP_LOGIN && CFG_EMAIL_SMTP_PASSWORD)
                {
                    $mail->SMTPAuth = true;
                    $mail->Username = CFG_EMAIL_SMTP_LOGIN;
                    $mail->Password = CFG_EMAIL_SMTP_PASSWORD;
                }
            }

            try
            {
               // Use custom sender if set
                if (strlen($sender_email_parsed))
                {
                    $mail->setFrom($sender_email_parsed, $sender_name_parsed);
                }
                // Otherwise: use current logged-in user (original Rukovoditel behavior)
                elseif (!empty($app_user['email']))
                {
                    $mail->setFrom($app_user['email'], $app_user['name']);
                }
                // Fallback: use global notification email
                elseif (CFG_EMAIL_USE_NOTIFICATION)
                {
                    $mail->setFrom(CFG_EMAIL_ADDRESS_FROM, CFG_EMAIL_NAME_FROM);
                }
                
                // ADD REPLY-TO
                if(strlen($reply_to_email_parsed))
                {
                    $mail->addReplyTo($reply_to_email_parsed, $reply_to_name_parsed);
                }

                foreach ($send_to as $to)
                {
                    if (is_numeric($to) && isset($app_users_cache[$to]['email']))
                    {
                        $mail->addAddress($app_users_cache[$to]['email'], $app_users_cache[$to]['name']);
                    }
                ...
                {
                    $email = trim($email);
                    if (filter_var($email, FILTER_VALIDATE_EMAIL)) $mail->addCC($email);
                }

                foreach ($bcc_list as $email)
                {
                    $email = trim($email);
                    if (filter_var($email, FILTER_VALIDATE_EMAIL)) $mail->addBCC($email);
                }
                ...
                if (is_array($attachments))
                {
                    ...
                    {
                        if (is_file($path)) $mail->addAttachment($path, $name);
                    }
                }
            ...
            {
                error_log('Email Rules (grouped) Error: ' . $e->getMessage());
            }
        }
        else
        {
            // INDIVIDUAL MODE – one private email per To recipient
            foreach ($send_to as $to_user)
            {
                $current_to = [];
                if (is_numeric($to_user) && isset($app_users_cache[$to_user]['email']))
                {
                    $current_to[] = $app_users_cache[$to_user]['email'];
                }
                elseif (filter_var($to_user, FILTER_VALIDATE_EMAIL))
                {
                    $current_to[] = $to_user;
                }

                if (empty($current_to)) continue;
                ...
                try
                {
                    // FIXED LOGIC: Same Sender checks as Grouped Mode
                    if (strlen($sender_email_parsed))
                    {
                        $mail->setFrom($sender_email_parsed, $sender_name_parsed);
                    }
                ...
                {
                    error_log('Email Rules (individual) Error: ' . $e->getMessage());
                }
            }
        }
    ...
    static function email_by_date()
    {
        ...
        while ($rules = db_fetch_array($rules_query))
        {
            ...
            foreach(explode(',',$number_of_days) as $day)
            {
                ...
                $day = (int)str_replace(array('+','-'),'',$day);
                $field_name = 'field_' . $date_fields_id;
                
                $item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($entities_id, '',false,array('fields_in_listing'=>$date_fields_id)) . " from app_entity_{$entities_id} e where FROM_UNIXTIME({$field_name},'%Y-%m-%d')=date_format({$use_function}(now(),INTERVAL {$day} DAY),'%Y-%m-%d')",false);
